...
- 通过 `MutationObserver` 捕获新增节点；首次接入聊天列表时补取末尾最多 30 条已有内容。
...
- 中置轨道最多占总轨道数的四分之一，且最多使用 3 条，避免重复评论占满整个显示区域。
...
- 引擎暂停、关闭、达到单帧生成上限或轨道暂时满载时会延后普通评论（最多 300 条）并保留受保护评论，恢复可用后继续按优先级调度；延后的实时评论会在实际生成时从画面右侧起跑，时间轴重建的历史评论才按原视频时间恢复位置。已超出可见周期或超过队列上限的队列项会被丢弃。
