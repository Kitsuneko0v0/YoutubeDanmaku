...
- 中置轨道最多使用 4 条。显示区域低于 100% 时，每条中置弹幕会临时在设置范围外补充一条滚动轨道（不超过播放器高度），因此不再受基础轨道四分之一的限制。中置弹幕结束后，对应拓展行立即停止接收新弹幕，但其中已有弹幕会保持原轨道正常播完；最底部空闲的拓展行随后自动回收。显示区域为 100% 时仍限制为总轨道数的四分之一。
