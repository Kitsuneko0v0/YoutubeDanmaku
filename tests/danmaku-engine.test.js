...
assert.equal(
  calculateBilibiliDanmakuX(1200, 120, 8, 14, 10, 2),
  870,
  '二倍速下媒体时间差应换算为真实屏幕运动时间'
);
...
const pauseHarness = {
  awaitingResumeFirstFrame: false,
  diagnostics: {
    pauseTransitions: 0,
    resumeTransitions: 0
  },
...
let tickDrainLimit = null;
let tickIncomingLimit = null;
const tickHarness = {
  active: [],
  awaitingResumeFirstFrame: true,
  diagnostics: {
    frameGapsOver25Ms: 0,
    frameGapsOver50Ms: 0,
  ...
  drainIncomingMessages(maxMessages) {
    tickIncomingLimit = maxMessages;
  },
  drainQueues(maxMessages) {
    tickDrainLimit = maxMessages;
    return 0;
  },
  enabled: true,
  fixedActive: [],
  ...
  publishDiagnostics() {},
  renderFrame() {},
  spawnDueTimelineMessages() {},
  tick() {}
};
const originalTickRequestAnimationFrame = global.requestAnimationFrame;
const originalTickCancelAnimationFrame = global.cancelAnimationFrame;
global.requestAnimationFrame = () => 17;
global.cancelAnimationFrame = () => {};
DanmakuEngine.prototype.tick.call(tickHarness, 1000);
assert.equal(tickDrainLimit, 1, '动画帧内最多应创建一条新弹幕的渲染表面');
assert.equal(tickIncomingLimit, 1, '聊天回放突发消息也应按每帧一条分散入场开销');
assert.equal(
  tickHarness.diagnostics.maxResumeFirstFrameDeltaMs,
  0,
  '暂停恢复后的首帧不应继承暂停期间的时间差而突然跳动'
);
cancelAnimationFrame(tickHarness.frameId);
global.requestAnimationFrame = originalTickRequestAnimationFrame;
global.cancelAnimationFrame = originalTickCancelAnimationFrame;
...
assert.equal(
  ...
  3,
  '非全屏占比下每条居中弹幕都应临时补充一条轨道'
);
assert.equal(
  getTemporaryLaneExpansion(100, 16, 16, 4),
  0,
  '百分百显示区域不应继续扩展到播放器之外'
);
assert.equal(
...
const fakeStage = {
  ...
  hidden: false,
  isConnected: true,
  parentElement: { clientHeight: 640 },
  style: stageStyle,
  append(child) {
...
assert.equal(
  canvasEngine.isMessageExpired({ videoTime: mediaTime - 10 }),
  false,
  '二倍速下队列过期判断不应把媒体时间差直接当作真实可见时长'
);
canvasEngine.setPlaybackRate(1);
assert.equal(
  canvasEngine.isMessageExpired({ videoTime: mediaTime - 10 }),
  true,
...
for (let index = 0; index < 100; index += 1) {
  assert.equal(
    canvasEngine.trySpawn(normalizeDanmakuMessage({ text: `满轨时应暂缓生成 ${index}` }, mediaTime)),
    false,
    '没有可用轨道时普通弹幕本帧不应强行生成'
  );
}
...
assert.ok(
  canvasEngine.active.every((item) => item.x === fakeStage.clientWidth),
  '超过帧预算而延后的评论应在实际生成时从右侧起跑'
);
...
assert.equal(
  canvasEngine.trySpawn(normalizeDanmakuMessage({
    id: 'timeline-restored-position',
    text: '时间轴恢复位置',
    videoTime: mediaTime - 1
  }, mediaTime)),
  true,
  '时间轴重建消息应能按历史时间生成'
);
assert.ok(
  canvasEngine.active[0].x < fakeStage.clientWidth,
  '时间轴重建仍应保留轨迹中途的历史位置'
);
canvasEngine.clearActive();
...
assert.equal(
  canvasEngine.getLaneCount(),
  canvasEngine.getBaseLaneCount() + 1,
  '非百分百显示区域出现一条中置弹幕后应临时增加一行'
);
assert.equal(
  fakeStage.style.height,
  `min(100%, calc(50% + ${canvasEngine.getLineHeight()}px))`,
  '中置弹幕出现后舞台高度应临时超出原显示占比一行'
);
assert.equal(
...
const retiringLane = canvasEngine.getBaseLaneCount();
const retiringMessage = normalizeDanmakuMessage({
  id: 'retiring-extra-lane-message',
  text: '拓展行中的滚动弹幕'
}, mediaTime);
const retiringSurface = canvasEngine.createRenderSurface(retiringMessage);
const retiringItem = {
  id: 'retiring-extra-lane-item',
  duplicateKey: null,
  lane: retiringLane,
  message: retiringMessage,
  surface: retiringSurface,
  width: retiringSurface.width,
  height: retiringSurface.height,
  x: 600,
  velocity: 120
};
canvasEngine.active.push(retiringItem);
const expandedStageHeight = fakeStage.style.height;
canvasEngine.removeFixedItem(canvasEngine.fixedActive[0]);
assert.equal(
  canvasEngine.getLaneCount(),
  canvasEngine.getBaseLaneCount(),
  '中置弹幕结束后拓展行应立即停止接收新增弹幕'
);
assert.equal(
  canvasEngine.getLaneState().length,
  canvasEngine.getBaseLaneCount(),
  '退役中的拓展行不应继续出现在新弹幕的可分配轨道中'
);
assert.equal(
  canvasEngine.getRenderLaneCount(),
  canvasEngine.getBaseLaneCount() + 1,
  '拓展行仍有滚动弹幕时应继续保留该行用于渲染'
);
assert.equal(
  fakeStage.style.height,
  expandedStageHeight,
  '中置弹幕结束时不应立即收缩仍有内容的拓展行'
);
canvasEngine.renderFrame();
assert.equal(retiringItem.lane, retiringLane, '退役行中的滚动弹幕不应突然上移');
assert.equal(
  canvasEngine.active.includes(retiringItem),
  true,
  '退役行中的滚动弹幕应继续保留到自然离场'
);
canvasEngine.removeItem(retiringItem);
assert.equal(
  fakeStage.style.height,
  '50%',
  '退役行清空后应恢复用户设置的显示区域占比'
);
...
canvasEngine.drainQueues();
assert.equal(
  canvasEngine.normalQueue[0]?.id,
  'temporarily-blocked-normal',
  '仍在可见时间窗内的普通消息应在轨道满载时延后重试'
);
canvasEngine.normalQueue = [];

canvasEngine.incomingNormalQueue.push({
  id: 'temporarily-blocked-incoming',
  protected: false,
  videoTime: mediaTime
});
canvasEngine.drainIncomingMessages();
assert.equal(
  canvasEngine.incomingNormalQueue[0]?.id,
  'temporarily-blocked-incoming',
  '动画帧预算内暂时无法入场的突发消息不应被直接丢弃'
);
canvasEngine.incomingNormalQueue = [];
